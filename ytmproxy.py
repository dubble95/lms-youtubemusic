...
# Load the YouTube Music cookie from the LMS plugin prefs file.
# The cookie is used for API calls (search, browse, library, Supermix)
# but NOT for stream resolution — the Android UA path used by yt-dlp
# must be cookie-free to avoid "Music Premium members only" errors.
_YTM_COOKIE = ""
_cookie_file = os.environ.get("YTM_COOKIE_FILE", "")
if not _cookie_file:
    # Try the default LMS plugin prefs location
    for _p in ("/var/lib/squeezeboxserver/prefs/plugin/ytm_cookies.txt",
               os.path.expanduser("~/var/lib/squeezeboxserver/prefs/plugin/ytm_cookies.txt")):
    ...
    try:
        ...
        with open(_cookie_file) as _f:
            for _line in _f:
                ...
                _parts = _line.split("\t")
                if len(_parts) >= 7:
                    _pairs.append(_parts[5] + "=" + _parts[6])
        _YTM_COOKIE = "; ".join(_pairs)
        logging.info("Loaded %d cookie pairs from %s", len(_pairs), _cookie_file)
    except Exception as _e:
        logging.warning("Failed to load cookies from %s: %s", _cookie_file, _e)
